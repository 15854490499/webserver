...
Fiber::Fiber() {
	...
	SetThis(this);
	if(getcontext(&m_ctx)) {
		LOG_ERROR << "getcontext error " << strerror(errno);
	}
	++gFiberCnt;
...
Fiber::Fiber(std::function<void()> cb, size_t stacksize) 
	...
	, m_cb(cb) {
		...
		if(getcontext(&m_ctx)) {
			LOG_ERROR << "getcontext error " << errno << " " << strerror(errno);
			exit(0);
		}
...
Fiber::~Fiber() {
	--gFiberCnt;
	if(m_stack) {
		LOG_ASSERT2(m_state == TERM, "Fiber State assertion");
...
void Fiber::reset(std::function<void()> cb) {
	LOG_ASSERT2(m_stack, "Fiber Stack assertion")
	LOG_ASSERT2(m_state == TERM, "Fiber State assertion");
	m_cb = cb;
	if(getcontext(&m_ctx)) {
		LOG_ERROR << "getcontext error " << errno << " " << strerror(errno);
...
void Fiber::swapIn() {
    SetThis(this);
    LOG_ASSERT2(m_state != EXEC, "Fiber State assertion");
    m_state = EXEC;
    if(swapcontext(&t_thread_fiber->m_ctx, &m_ctx)) {
		LOG_ERROR << "swapcontext error " << errno << " " << strerror(errno);
		exit(0);    
	}
}
...
//切换到后台执行
void Fiber::swapOut() {
    LOG_ASSERT2(m_state == EXEC || m_state == TERM, "Fiber State assertion");
...
Fiber::ptr Fiber::GetThis() {
    ...
    LOG_ASSERT2(t_fiber == main_fiber.get(), "running Fiber assertion");
...
void Fiber::FiberEntry()
{
    ...
    LOG_ASSERT2(curr != nullptr, "running Fiber assertion");
    curr->m_cb();
    curr->m_cb = nullptr;
    curr->m_state = TERM;
    ...
    curr.reset();
    ptr->swapOut();
    LOG_ASSERT2(false, "never reach here");
}
